...
if step != 99:
    if get_unique_user_count(get_sheet_data()) >= 20:
...
# STEP 7 — DASHBOARD
    ...
    with tm["📊 Content"]:
        ...
        with ce:
            ...
            days_all = df_posts[(df_posts["Day"].isin(DAG_EN)) & (df_posts["Weergaven"]>0)].groupby("Day").agg(G=("Engagement_pct","median"), count=("Engagement_pct","count")).reset_index(); days = days_all[days_all["count"] >= 3]
        ...
        with cr:
            ...
            dr_all = df_posts[(df_posts["Day"].isin(DAG_EN)) & (df_posts["Weergaven"]>0)].groupby("Day").agg(G=("Weergaven","median"), count=("Weergaven","count")).reset_index(); dr = dr_all[dr_all["count"] >= 3].sort_values("G",ascending=True)
